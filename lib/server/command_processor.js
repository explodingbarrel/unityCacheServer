...
class CommandProcessor extends Duplex {
    ...
    constructor(cache) {
        ...
        this._whitelistEmpty = (!this._putWhitelist);// kabam-: ip white list regex implementation.
   ...
   async _quit(err) {
    ...
    async _onPut(type, size) {
        ...
        if (this._isWhitelisted(this._trx.clientAddress)) {
            // kabam-: debug print for white listed ips.
