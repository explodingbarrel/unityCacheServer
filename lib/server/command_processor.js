...
class CommandProcessor extends Duplex {
    ...
    constructor(cache) {
        ...
        this._whitelistEmpty = (!this._putWhitelist);// kabam-wchow: ip white list regex implementation.
     ...
     */
    _isWhitelisted(ip) {
        // kabam-wchow : regex.test: begin : do regex check here
        var ipstr = this._putWhitelist || '.*';
        var iplist = new RegExp(ipstr, 'i');
        return this._whitelistEmpty || iplist.test(ip);
        // kabam-wchow : regex.test: end : do regex check here
    }
   ...
   async _quit(err) {
    ...
    async _onPut(type, size) {
        ...
        if (this._isWhitelisted(this._trx.clientAddress)) {
            // kabam-wchow: debug print for white listed ips.
            helpers.log(consts.LOG_DBG, `PUT from whitelisted IP: ${this._trx.clientAddress}`);
