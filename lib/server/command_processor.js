...
class CommandProcessor extends Duplex {
    ...
    constructor(cache) {
        ...
        this._whitelistEmpty = (!this._putWhitelist);// wchow ip white list
     ...
     * @private
     */
    _isWhitelisted(ip) {//todo wchow do regex here
        // wchow do regex check here
        var ipstr = this._putWhitelist || '.*';
        var iplist = new RegExp(ipstr, 'i');
        return this._whitelistEmpty || iplist.text(ip);
   ...
   async _quit(err) {
    ...
    async _onPut(type, size) {
        ...
        if (this._isWhitelisted(this._trx.clientAddress)) {
            //todo wchow debug code for ip white list
