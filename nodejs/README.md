...
This is version 4.3.1, official binaries from http://nodejs.org

IMPORTANT NOTE FOR EMSCRIPTEN:
On linux and mac, you can just do:
`NODE_JS = ['node', '--stack_size=8192']` or such (in ~/.emscripten). On windows, you will also need `--max-stack-size=8192`, and also to run `editbin /stack:33554432 node.exe`.
(this node.exe is modified already).
